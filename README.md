...
سيرفر بسيط (Node.js + Express) بيشتغل كـ"دماغ" لبلجن AI Animator. البلجن بيبعتله وصف الحركة وأسماء المفاصل، والسيرفر بيتواصل مع Claude (Anthropic) ويرجّع خطة Keyframes جاهزة.
...
npm install
cp .env.example .env
# افتح .env وحط مفتاح Anthropic API عندك بدل القيمة الوهمية
npm start
```
...
- **مفتاح Anthropic API خاص فيك وسري** - لا تحطه أبدًا داخل كود البلجن نفسه (Lua) لأنه بيترفع مع أي مشروع تنشره. حطه بس بإعدادات Environment على السيرفر.
- الخطة المجانية على Render بتنام (sleep) بعد فترة عدم استخدام، فأول طلب بعد فترة راحة ممكن ياخد شوي وقت أطول (cold start).
- لو حبيت تستخدم خدمة AI ثانية غير Anthropic، عدّل فقط الجزء يلي بيستدعي `api.anthropic.com` جوا `server.js`.
